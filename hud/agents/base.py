...
class MCPAgent(ABC):
    ...
    async def run(
        ...
        *,
        ...
        try:
            # Try to use tau2-bench conversation loop if dependencies are available
            # This enables multi-turn conversations with user simulator
            try:
                ...
                result = await _run_conversation_loop(
                    self, text_to_blocks(ctx.prompt), max_steps=max_steps
                )
            except (ImportError, ModuleNotFoundError):
                # tau2 modules not available - use standard loop
                logger.debug("tau2-bench modules not available, using standard agent loop")
                result = await self._run_context(text_to_blocks(ctx.prompt), max_steps=max_steps)
